...
# Calculating percentage decline in the total population between the 1st and 4th quarters
percent_decrease_N <- list()
for(alt in 1:num_alternatives) {
  Q1_total_Ns <- alt_data[[alt]]$N[alt_data[[alt]]$size_class == "total" & alt_data[[alt]]$Quarter == 1]
  Q4_total_Ns <- alt_data[[alt]]$N[alt_data[[alt]]$size_class == "total" & alt_data[[alt]]$Quarter == 4]
  percent_decrease_N[[alt]] <- (Q1_total_Ns - Q4_total_Ns)/Q1_total_Ns
}
...
## Adding N.sum mean and 95% CIs to alt_results
estimated_N_sum <- readRDS(here("Results", "DoD_final_report", "all_estimated_N_sum_list.rds"))
for(alt in 1:num_alternatives) {
  alt_N_sum <- melt(estimated_N_sum[[alt]], id.vars = colnames(estimated_N_sum[[alt]][[1]]))
  alt_N_sum$Quarter <- c(1:4)
  alt_N_sum$size_class <- "total"
  colnames(alt_N_sum)[c(1:4)] <- c("N", "lower_CI", "upper_CI", "variant")
  alt_N_sum_melted <- alt_N_sum[, colnames(alt_results[[1]])]
  alt_results[[alt]] <- rbind(alt_results[[alt]], alt_N_sum_melted)
}

# Melting alternatives together to plot
...
for(alt in 1:num_alternatives) {
  ...
  for(variant in 1:num_variants) {
    ...
    final_density$simulated[[alt]][[variant]] <- data[data$Quarter == tail(obs_quarters, n = 1),c(2:6)]/area_size
    final_density$estimated[[alt]][[variant]] <- results[results$Quarter == tail(obs_quarters, n = 1),c(2:6)]/area_size
  }
}
...
density_threshold_plot <- ggplot(all_summed_density[all_summed_density$source == "simulated",]) +
  geom_point(aes(x = alternative, y = threshold_prob, color = alternative), size = 2) +
  geom_hline(aes(yintercept = 0.95)) +
  facet_grid(cols = vars(type_of_N), labeller = labeller(type_of_N = plot_labels$type_of_N)) +
  theme_bw() +
  scale_color_manual(values = colors$alt,
                     labels = plot_labels$alternative) +
  theme_bw() + 
