...
erad_coverage <- list()
erad_coverage$ADS <- 1
erad_coverage$visual <- (4*num_teams$visual)/113
...
N_list <- round(rnorm(n_inter, 50, 10))*area_size
...
S <- length(size_class_names) # number of size classes
Q <- length(unique(unlist(erad_quarters[obs_methods]))) # Primary sampling periods (quarters)
# When two methods are used, I is double the length of sampling days:
#I <- rep(max(length(erad_days[[obs_methods[1]]]), length(erad_days[[obs_methods[2]]]))*2, Q)
# With only one method, I is just the max number of days for that method:
...
for(variant in 1:1) {
  # Mean N estimates vs simulated real N
est_v_sim_N_plot_list[[variant]] <- estimated_N_plots(jags_output = jags_output_list[[variant]],
                       erad_quarter_results = erad_results_list[[variant]],
                       erad_quarters = erad_quarters)
}
...
plot_labels <- list()
plot_labels$alternative <- c("alt_1" = "Alternative 1",
                             "alt_2" = "Alternative 2")
...
for(alt in 1:num_alternatives) {
  variant_melted <- list()
  for(variant in 1:num_variants) {
   variant_melted[[variant]] <- melt(alt_model_metrics[[alt]][[variant]]$summed_data[,-6], id.vars = "Quarter")
...
final_density_plot <- ggplot(all_final_density) +
...
effort_plot_2 <- ggplot(all_effort, aes(fill = method, x = week)) +
  geom_bar(stat = "count", position = "dodge") +
  facet_grid(quarter ~ alternative) +
