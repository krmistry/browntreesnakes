...
for(P in 1:length(starting_density)) { # Starting pop permutation (low, medium or high)
  ...
  for(D in 1:length(starting_size_dist)) { # Starting size distribution permutation (more small or more large)
    ...
    for(variant in 2:length(N_reps)) { # Start running through the 100 starting pop variants for the selected permutation
...
# Registering the libraries needed 
...
results <- foreach(variant = 1:4, .combine = list)  %dopar% {
