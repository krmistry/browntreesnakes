...
-   Thresholds:
    ...
    -   Estimated mean density of entire population is =\< 0 snakes/ha for at least 2 quarters
        ...
        -   If met: stop all methods for 2 quarters, then do low effort visual survey (3 weeks with 2 teams, at the beginning of the quarter) in the next 2 quarters to estimate the population
...
strat_2_threshold_fun <- function(mean_N_df,
  ...
  upper_3_mean_density <- sum(last_quarter_means$N[last_quarter_means$size_class != size_class[1]])/area_size
  # Total population
...
# Threshold 3 days (no effort in first 2 quarters, with visual effort in the next 2)
method_options$threshold_3$erad_days <- list()
for(quarter in 1:4) {
  method_options$threshold_3$erad_days[[quarter]] <- list()
}
names(method_options$threshold_3$erad_days) <- paste0("quarter_", c(1:4))
# Visual survey: low effort (3 weeks, 2 teams) per quarter - surveying every other day
method_options$threshold_3$erad_days$quarter_3$visual <- seq(2, (7*3-1), 2)
method_options$threshold_3$erad_days$quarter_4$visual <- seq(2, (7*3-1), 2)
...
method_options$threshold_3$primary_sampling_period <- seq(2, (7*3-1), 2)
...
estimation_inputs <- estimation_inputs_fun(erad_quarter_results,
                                           new_methods$erad_days)
...
# Example
N_test <- c(10, 11)*area_size
n_reps <- length(N_test)

results <- foreach(i = 1:n_reps, .combine = list)  %dopar% {
  quarter_operations(initial_N = N_test[i],
                     initial_size_dist = c(0.4, 0.1, 0.1, 0.4), 
                     p_g = g_density_prob,
                     lambda = lambda,
                     total_quarters = erad_quarter_time_step,
