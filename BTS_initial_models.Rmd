...
timeseries <- as.data.frame(matrix(NA, nrow = 80, ncol = n_bins+1))
...
# Adding vector to identify years, since the time step is less than a year (3 months)
timeseries$year <- rep(1:20, each = 4)
...
tm_plot <- melt(timeseries[, c(1:4, 6)], id.vars = "year")
colnames(tm_plot)[2:3] <- c("size", "pop")
#tm_plot$Year <- c(1:20)
...
-   Loop from 2 - 80 time steps OR condition met:

    -   Generate new population for time step, using equations in first step above

    -   Using new population, perform removal step, using equation in second step above
...
timeseries_rm <- timeseries[0,]
timeseries_rm[1, ] <- c(rep(250, 4), 1000, 1)
...
# Melt matrix to make plotting easier
tm_rm_plot <- melt(timeseries_rm[, c(1:4, 6)], id.vars = "year")
colnames(tm_rm_plot)[2:3] <- c("size", "pop")

ggplot(tm_rm_plot, aes(fill = size, y = pop, x = year)) +
...
timeseries_raw <- timeseries[0,]
timeseries_raw[1, ] <- c(rep(2500, 4), 10000, 1)
...
# Adding year to both raw and observation timeseries
...
tm_obs_plot <- melt(timeseries_obs[, c(1:4, 6)], id.vars = "year")
colnames(tm_obs_plot)[2:3] <- c("size", "pop")
...
tm_raw_obs_plot <- melt(rbind(timeseries_raw[, -5], timeseries_obs[, -5]), id.vars = c("year", "data_type"))
colnames(tm_raw_obs_plot) <- c("year", "data_type", "size", "pop")
tm_raw_obs_plot <- drop_na(tm_raw_obs_plot, "pop")


# Plot
ggplot(tm_raw_obs_plot, aes(fill = data_type, color = size, y = pop, x = year)) +
    geom_bar(position="dodge", stat="identity")

```

\*ALL OF THE GGPLOTS ARE TAKING ONLY THE FIRST POPULATION FROM EACH YEAR - NEED TO FIX THIS\*
