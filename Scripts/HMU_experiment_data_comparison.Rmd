...
# Converting the effort dates into numbered days, with the first setdate (2/6/23) acting as the first day of the first quarter
...
for(size_var in 3:length(size_dist_variants)) {
  ...
  saveRDS(size_var_IBM_results[[size_var]], file = here("Results", "HMU", paste0("size_dist_var_", size_var, ".rds")))
...
for(rep in 1:num_replicates) {
 rep_observed <- IBM_results_list[[rep]]$all_observed[unlist(lapply(IBM_results_list[[rep]]$all_observed, length) != 0)]
 observed_list <- bind_rows(rep_observed)
  ...
  for(method in 1:length(observed_methods)) {
    all_observed_list[[rep]][[method]] <- as.data.frame(matrix(NA, nrow = 4, ncol = 2)) 
    colnames(all_observed_list[[rep]][[method]]) <- c("size_class", "removed")
    all_observed_list[[rep]][[method]]$size_class <- size_class_names
    for(size in 1:length(size_class_names)) {
      all_observed_list[[rep]][[method]]$removed[size] <- nrow(observed_list[observed_list$size_category == size_class_names[size] & observed_list$method == observed_methods[method], ])
    }
  }
    names(all_observed_list[[rep]]) <- observed_methods
}
all_observed <- melt(all_observed_list, id.vars = colnames(all_observed_list[[1]][[1]]))
colnames(all_observed)[c(3:4)] <- c("method", "rep")
all_observed$rep <- paste0("replicate_", all_observed$rep)
all_observed$quarter <- 1
...
HMU_observed <- list()
for(method in 1:length(observed_methods)) {
  HMU_observed[[method]] <- as.data.frame(matrix(NA, nrow = 4, ncol = 2))
  colnames(HMU_observed[[method]]) <- c("size_class", "removed")
  HMU_observed[[method]]$size_class <- size_class_names
  for(size in 1:length(size_class_names)) {
    HMU_observed[[method]]$removed[size] <- nrow(all_HMU_observed[all_HMU_observed$Quarter == 1 & all_HMU_observed$size_class == size_class_names[size] & all_HMU_observed$method == observed_methods[method],])
  }
}
names(HMU_observed) <- observed_methods

all_HMU_summed <- melt(HMU_observed, id.vars = colnames(HMU_observed[[1]]))
colnames(all_HMU_summed)[3] <- "method"
all_HMU_summed$quarter <- 1
all_HMU_summed$source <- "HMU_data"

# Bar plot
observed_plot <- ggplot() +
  geom_violin(data = all_observed, aes(fill = size_class, y = removed, x = size_class)) +
