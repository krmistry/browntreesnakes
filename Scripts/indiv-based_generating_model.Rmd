...
daily_gompertz_growth_fun_2 <- function(snake, 
                                      density_prob,
                                      gompertz_coefs) {
   if (rbinom(1, 1, prob = density_prob) == 1) {
     ...
     # Snake grows
     snake$SVL <- snake$SVL*exp(growth_coefs[1,]/365 + (growth_coefs[2,]/365)*log(snake$SVL/700))
   } else {
     snake$SVL <- snake$SVL
   }
  return(snake)
...
growth_curves$Females <- as.data.frame(matrix(NA, nrow = 10*365, ncol = 6))
colnames(growth_curves$Females) <- c("Day", "25th", "50th", "75th", "90th", "95th")
growth_curves$Females$Day <- c(1:(10*365))
growth_curves$Females[1, -1] <- 350
for(t in 1:length(tau)) {
  coefs <- gompertz_coefficients[[t]]$F
  for(day in 2:(10*365)){
  growth_curves$Females[day, t+1] <- growth_curves$Females[day-1, t+1]*exp(coefs[1] + coefs[2]*log(growth_curves$Females[day-1,t+1]/700))
  }
}
# Male growth curves
growth_curves$Males <- as.data.frame(matrix(NA, nrow = 10*365, ncol = 6))
colnames(growth_curves$Males) <- c("Day", "25th", "50th", "75th", "90th", "95th")
growth_curves$Males$Day <- c(1:(10*365))
growth_curves$Males[1, -1] <- 350
for(t in 1:length(tau)) {
  coefs <- gompertz_coefficients[[t]]$M
  for(day in 2:(10*365)){
  growth_curves$Males[day, t+1] <- growth_curves$Males[day-1, t+1]*exp(coefs[1] + coefs[2]*log(growth_curves$Males[day-1,t+1]/700))
  }
}
...
ggplot(all_growth_curves, aes(x = Day/365, y = SVL, color = Growth_quant)) +
  geom_line() +
  # geom_hline(yintercept = 967.5, color = "red") + # average size for female sexual maturity
  # geom_hline(yintercept = 985, color = "darkblue") + # average size for male sexual maturity
  facet_grid(rows = vars(Sex)) +
  theme_bw() +
  labs(x = "Year", y = "SVL (mm)", color = "Growth \nQuantile")
  

# Calculating time to reach sexual maturity - time to reach 910 
    ...
    -   Estimated from multiple study sites around the western and northern parts of the island, surveyed from 1985 - 1990:
        ...
        -   Using CAPTURE software and mark-recapture data, they calculated these densities in different study areas:
            ...
            -   Central Guam: 63/ha (45 - 108) for a 2 ha study area using Schumacher technique, CAPTURE estimated 58/ha (18 - 97) - although the CAPTURE assumptions were violated, but in small ways
                ...
                -   Central had snakes for longer, so during this study period, snakes had probably already peaked (birds and bats were already depleted)
...
for(snake in 1:length(SVL)) {
  mortality[snake] <- mortality_prob_erad_methods[[1]][1]*diet_trans_fun(SVL[snake])*mortality_decay_fun(SVL[snake])
}
...
### Three arrays needed for inputs into the removal-based estimation model:
# - Total captured, in method x size class arrays, in a list for each day 
# - Effort array, in method x day format
# - Vector of the number of days betwen data collection dates
### Using all visual survey and trapping results for this initial experiment (in the future, 
### I probably won't use all the data collected

# Transforming collection data into arrays
# First, put all of the results data in one dataframe
all_erad_results <- erad_quarter_results$all_observed[unlist(lapply(erad_quarter_results$all_observed, length) != 0)] %>%
  melt(., id.vars = colnames(.[[1]]))

# Adding size class column to make it easier to partition
for(snake in 1:nrow(all_erad_results)) {
  all_erad_results$size_class[snake] <- size_class_fun(all_erad_results$SVL[snake])
}

captured_list <- list()
for(quarter in 1:length(unique(all_erad_results$quarter))) {
  captured_list[[quarter]] <- list()
  for(day in 1:length(unique(all_erad_results$day))) {
    captured_list[[quarter]][[day]] <- as.data.frame(matrix(0, nrow = 2, ncol = 4))
    colnames(captured_list[[quarter]][[day]]) <- size_class_names
    rownames(captured_list[[quarter]][[day]]) <- erad_methods[c(2:3)]
    day_results <- all_erad_results[all_erad_results$quarter == unique(all_erad_results$quarter)[quarter] & all_erad_results$day == unique(all_erad_results$day)[day],]
    for(method in erad_methods[c(2:3)]) {
      for(size in size_class_names) {
      captured_list[[quarter]][[day]][method, size] <- nrow(day_results[day_results$size_class == size & day_results$method == method, ])
      }
    }
  }
}
full_list <- c(captured_list[[1]], captured_list[[2]], captured_list[[2]], captured_list[[4]])

### Configuring effort into a dataframe, 
all_effort_results <- erad_quarter_results$all_effort[unlist(lapply(erad_quarter_results$all_effort, length) != 0)] %>%
  melt(., id.vars = colnames(.[[1]])) %>%
  .[.$method %in% erad_methods[c(2:3)],]

## Full effort amounts, used to standardize
daily_effort <- c("visual" = 4, "trap" = 12)

## Standardizing effort 
for(row in 1:nrow(all_effort_results)) {
  all_effort_results$std_effort[row] <- all_effort_results$effort[row]/daily_effort[all_effort_results$method[row]]
