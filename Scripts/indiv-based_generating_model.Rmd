...
for(day in 1:(time_step-1)) {
  pop <- timeseries_pop[[day]]
  for(snake in 1:nrow(pop)) {
    if (rbinom(1, 1, prob = g_density_prob) == 1) {
...
names(timeseries_pop) <- paste0("day_", c(1:time_step))
...
new_timeseries_pop <- rbind(timeseries_pop[[length(timeseries_pop)]], offspring)
...
for(snake in 1:nrow(timeseries_pop[[1]])) {
  if(timeseries_pop[[1]]$SVL[snake] <= 850) {
    if (rbinom(1, 1, prob = N_mortality[1]) == 1) { 
      dead_snakes[snake] <- timeseries_pop[[1]]$ID[snake]
    }
  } else if (timeseries_pop[[1]]$SVL[snake] > 850 & timeseries_pop[[1]]$SVL[snake] <= 950) {
    if (rbinom(1, 1, prob = N_mortality[2]) == 1) { 
      dead_snakes[snake] <- timeseries_pop[[1]]$ID[snake]
    }
  } else if (timeseries_pop[[1]]$SVL[snake] > 950 & timeseries_pop[[1]]$SVL[snake] <= 1150) {
    if (rbinom(1, 1, prob = N_mortality[3]) == 1) { 
      dead_snakes[snake] <- timeseries_pop[[1]]$ID[snake]
    } 
  } else if (timeseries_pop[[1]]$SVL[snake] > 1150) {
...
new_timeseries_pop_2 <- timeseries_pop[[1]][!timeseries_pop[[1]]$ID %in% dead_snakes,]
