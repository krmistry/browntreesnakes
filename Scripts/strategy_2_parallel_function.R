## Trying out making the dynamic process into a single function, and then running 
## parallel loop with just that
...
strat_2_parallel_fun <- function(P, 
                                 D, 
                                 final_time_step,
                                 variant) {
  ...
  saveRDS(estimation_effort_record, file = here("Results", "alt_strategies", "Strategy_two", "IBM", 
                                                paste0("effort_record_start_pop_", names(starting_pop)[P], "_", 
                                                       names(starting_size_dist)[D], "_variant-", variant, ".rds")))
  saveRDS(IBM_all_quarters, file = here("Results", "alt_strategies", "Strategy_two", "IBM", 
