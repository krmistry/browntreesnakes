...
s3 ~ dunif(0.9991, 0.9999)
s4 ~ dunif(0.9991, 0.9999)
f2 ~ dgamma(1,0.3)
f3 ~ dgamma(1,0.3)
f4 ~ dgamma(1,0.3)
  ...
  for(i in 1:I) {
    ...
    for(k in 1:K) {
       for(j in 1:J) {
          ...
          #Y[j,k,i,t] ~ dbin(0.8, 1000)
       }
      
      for(j in 2:J) {
          N[j,k,i,t] <- N[j-1,k,i,t] - Y[j-1,k,i,t]
          #N[j,k,i,t] <- 2000 - Y[j-1,k,i,t]
      }
        N[1,k,i+1,t] <- N[J,k,i,t]
      }
  }
  for(k in 1:K) {
...
## Adding total # of captured snakes and total # of alive snakes in each size
...
# Combining all size classes to get a total N for each source category
total_N <- as.data.frame(matrix(NA, nrow = 4, ncol = 3))
colnames(total_N) <- c("Quarter", "Total_N", "source")
for(quarter in 1:length(observed_erad_quarters)) {
  total_N$Total_N[quarter] <- 
}
