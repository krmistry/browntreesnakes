...
##################### Going back to set up for jags model ###########
...
est_total_N <- vector()
for(quarter in 1:length(observed_erad_quarters)) {
  real_total_N[quarter] <- sum(real_N_long$N[real_N_long$Quarter == observed_erad_quarters[quarter]])
  est_total_N[quarter] <- sum(mean_N_long$N[mean_N_long$Quarter == observed_erad_quarters[quarter]])
}
